...
supply = read.csv('Fport September.csv')
...
ui <- fluidPage(
  ...
  sidebarPanel(
    ...
    selectInput('x', 'X', choices = nms, selected = "squad"),
    selectInput('y', 'Y', choices = nms, selected = "omd_dollar"),
...
server <- function(input, output) {
  ...
  output$trendPlot <- renderPlotly({
    ...
    p <- ggplot(supply, aes_string(input$x, input$y, color = input$color)) + 
      geom_boxplot() + theme(axis.text.x=element_text(size=6,angle=90)) + xlab(input$x) + ylab(input$y)
